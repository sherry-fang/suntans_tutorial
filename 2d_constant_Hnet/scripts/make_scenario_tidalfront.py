...
def make_suntans(suntanspath):
    ...
    wave_period=12.00*3600.
    U0 = 0.0        # Barotropic velocity [m/s]
    ...
    # Size of domain
    L = 300e3 # 370e3  # Domain length [m]
    W = 3e3 # Domain width [m]
    H = -220 # Maximum depth
    ...
    x0 = np.array([0, 100e3,L])
    y0 = np.array([H,H,H+200])
    #x0 = np.array([0, 200e3,L])  #Slope =0.2%
    #y0 = np.array([H,H,H+200])
    #x0 = np.array([0,L])  #Slope =0.05%
    #y0 = np.array([H+50,H+200])


    # Boundary condition time
    starttime = '20000601.000000'
    endtime = '20000930.000000'
    dt = 3600
    ...
    F_bathy = interp1d(x, -h_smooth, kind=3,fill_value="extrapolate")
    ...
    grd.mark[indleft]= 2 #2is openning (06/06/22 Sherry noted)
    grd.mark[indright]=1 #2 1 is closed
   ...
   # Write the boundary file
    ...
    IC.S[:,:,:] = S0 # set S = 1 in the lower water column
